...
namespace EntJoySample.ManyJobsBenchTest
{
    ...
    public sealed unsafe class ManyJobsBenchSample : IDisposable
    {
        ...
        public ManyJobsBenchSample()
        {
            ...
            _realData = new NativeArray<float>[8];
            for (int k = 0; k < _realData.Length; k++)
            {
                _realData[k] = new NativeArray<float>(1_000_000, Allocator.Persistent);
                for (int i = 0; i < _realData[k].Length; i++) _realData[k][i] = i * 0.5f;
            }
        ...
        public void Dispose()
        {
            ...
            foreach (var d in _realData) d.Dispose();
        }
        ...
        /// <summary>带"帧前休眠"的 case：把 worker 逼入 park 后再测本帧调度/完成。</summary>
        private static BenchCase CaseWithGap(string label, int n, double gapMs, Action<JobHandle[]> schedule)
        {
            var c = Case(label, n, schedule);
            c.PreFrame = () => Thread.Sleep((int)Math.Round(gapMs));
            return c;
        }
        ...
        private List<BenchCase> BuildCases()
        {
            ...
            {
                for (int i = 0; i < 8; i++) h[i] = realJobs[i].Schedule(1_000_000, 0);
            }));
            cases.Add(CaseWithGap("RealPar·1M x8 (pre-sleep 3ms)", 8, 3.0, h =>
            {
                for (int i = 0; i < 8; i++) h[i] = realJobs[i].Schedule(1_000_000, 0);
            }));
            cases.Add(CaseWithGap("NativeImplicit·RealPar·1M x8 (pre-sleep 3ms)", 8, 3.0, h =>
            {
                NativeJobScheduler.SetImplicitBatchEnabled(true);
                try
                {
                    for (int i = 0; i < 8; i++) h[i] = realJobs[i].Schedule(1_000_000, 0);
                    NativeJobScheduler.EndFrame();     // pending 统一提交 + 单次唤醒
                }
                finally
                {
            ...
            cases.Add(CaseWithGap("Batch·RealPar·1M x8 (pre-sleep 3ms)", 8, 3.0, h =>
            {
                using var b = new BatchScope();
                for (int i = 0; i < 8; i++) b.AddParallelFor(ref realJobs[i], 1_000_000, 0);
                var hs = b.Submit();
                for (int i = 0; i < hs.Length; i++) h[i] = hs[i];
            }));
