...
> 技术细节（分配器 / 调度 / 托管开销）见 [gridsearch/](gridsearch/) 的 01–05 系列，历史快照见 [archive/](archive/README.md)。
...
- **活动入口在 `06_HotFieldHandle/Program.cs`**（遵循 §3.2 约定，`Main` 只做初始化 + 运行）。`05_Algorithms/GridSearch`（`TestGridSearch.cs` + `GridSearch2D.cs`）整体 `/* */` 注释停用（2026-08），设计见 [06-HotFieldHandle设计](06-HotFieldHandle设计.md)。
