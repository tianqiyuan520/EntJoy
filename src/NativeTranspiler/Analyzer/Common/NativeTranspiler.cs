    ...
    public static partial class NativeTranspiler
    {
        ...
        public static bool IsEntJoyNativeContainerType(ITypeSymbol? type)
        {
            if (type is INamedTypeSymbol named && named.IsGenericType)
            {
                ...
                if (ns == Config.NamespaceEntJoyCollections)
                {
                    var typeName = named.Name;
                    if (typeName == Config.NativeArray || typeName == Config.NativeList || typeName == Config.UnsafeList)
                        return true;
                }
            }
            return false;
        ...
        /// <summary>
        /// 命名空间 + 名字双校验：判断类型是否为 EntJoy.Collections 下指定的容器成员名
        /// （NativeArray / NativeList / UnsafeList）。用于替代散落各处"只看短名"的
        /// <c>.Name == Config.X</c> 判断，避免把用户自定义同名类型误判为 EntJoy 容器。
        /// </summary>
        public static bool IsEntJoyContainerNamed(ITypeSymbol? type, string memberName)
        {
            return IsEntJoyNativeContainerType(type) &&
                   (type as INamedTypeSymbol)?.Name == memberName;
        }
        ...
        public static bool IsEntJoyPredefinedType(ITypeSymbol type)
        {
            ...
            return ns == Config.NamespaceEntJoyCollections;
        }
