...
namespace NativeTranspiler.Analyzer
{
    public static class BindingsGenerator
    {
            ...
            Compilation compilation)
        {
            ...
            sb.AppendLine("        private static IntPtr s_nativeDllHandle = IntPtr.Zero;");
            sb.AppendLine("        private static bool s_resolverInstalled = false;");
            sb.AppendLine("        private static readonly object s_resolverLock = new object();");
            sb.AppendLine();
            sb.AppendLine("        private static void EnsureNativeDllResolver()");
            sb.AppendLine("        {");
            sb.AppendLine("            lock (s_resolverLock)");
            sb.AppendLine("            {");
            sb.AppendLine("                if (s_resolverInstalled) return;");
            sb.AppendLine("                NativeLibrary.SetDllImportResolver(typeof(NativeExports).Assembly, ResolveNativeDllImport);");
            sb.AppendLine("                s_resolverInstalled = true;");
            sb.AppendLine("            }");
            sb.AppendLine("        }");
            sb.AppendLine();
            sb.AppendLine("        private static IntPtr ResolveNativeDllImport(string libraryName, Assembly assembly, DllImportSearchPath? searchPath)");
            sb.AppendLine("        {");
            sb.AppendLine("            if (!string.Equals(libraryName, \"NativeDll\", StringComparison.OrdinalIgnoreCase) &&");
            sb.AppendLine("                !string.Equals(libraryName, \"NativeDll.dll\", StringComparison.OrdinalIgnoreCase))");
            sb.AppendLine("                return IntPtr.Zero;");
            sb.AppendLine();
            sb.AppendLine("            if (s_nativeDllHandle != IntPtr.Zero) return s_nativeDllHandle;");
            sb.AppendLine();
            sb.AppendLine("            string baseDir = AppContext.BaseDirectory;");
            sb.AppendLine("            if (!string.IsNullOrEmpty(baseDir))");
            sb.AppendLine("            {");
            sb.AppendLine("                string candidate = Path.Combine(baseDir, \"NativeDll.dll\");");
            sb.AppendLine("                if (NativeLibrary.TryLoad(candidate, out var handle))");
            sb.AppendLine("                {");
            sb.AppendLine("                    s_nativeDllHandle = handle;");
            sb.AppendLine("                    return handle;");
            sb.AppendLine("                }");
            sb.AppendLine("            }");
            sb.AppendLine();
            sb.AppendLine("            if (NativeLibrary.TryLoad(\"NativeDll\", assembly, searchPath, out var fallback))");
            sb.AppendLine("            {");
            sb.AppendLine("                s_nativeDllHandle = fallback;");
            ...
            sb.AppendLine("        {");
            sb.AppendLine("            EnsureNativeDllResolver();");
            foreach (var jobStruct in jobStructs)
            {
        ...
        private static void GenerateJobScheduleMethod(StringBuilder sb, INamedTypeSymbol jobStruct, Compilation compilation)
        {
            ...
            if (isParallelFor || isFor)
            {
                sb.AppendLine($"            int actualBatchSize = innerBatchCount > 0");
                sb.AppendLine($"                ? Math.Min(arrayLength, innerBatchCount)");
                sb.AppendLine($"                : Math.Min(arrayLength, Math.Max(1, 32 * Environment.ProcessorCount));");
