...
#include "SharpNative_Job__global_namespace__MoveSystemJobEntityIspc_Execute_mt_ispc.h"
#include <thread>
...
HEAD void CALLINGCONVENTION SharpNative_Job__global_namespace__MoveSystemJobEntityIspc_Execute_EntityBatchAdapter(void* context, const EntityBatchData* __batches, int __batch_start, int __batch_count)
{
    ...
    for (int __batch_index = __batch_start; __batch_index < __batch_end; ++__batch_index)
    {
        ...
        auto* position_ptr = reinterpret_cast<ispc::Position*>(__batchData->componentArrays[0]);
        auto* velocity_ptr = reinterpret_cast<ispc::Vel*>(__batchData->componentArrays[1]);
        ispc::SharpNative_Job__global_namespace__MoveSystemJobEntityIspc_Execute_mt_impl(position_ptr, velocity_ptr, __batchData->entityCount, dt_ptr, std::thread::hardware_concurrency());
    }
}

HEAD void* CALLINGCONVENTION Get_SharpNative_Job__global_namespace__MoveSystemJobEntityIspc_Execute_EntityBatchAdapterPtr()
{
    return (void*)SharpNative_Job__global_namespace__MoveSystemJobEntityIspc_Execute_EntityBatchAdapter;
}
