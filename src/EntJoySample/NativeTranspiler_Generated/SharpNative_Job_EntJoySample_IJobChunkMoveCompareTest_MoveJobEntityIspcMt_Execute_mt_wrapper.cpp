...
#include "SharpNative_Job_EntJoySample_IJobChunkMoveCompareTest_MoveJobEntityIspcMt_Execute_mt_ispc.h"
#include <thread>
...
HEAD void CALLINGCONVENTION SharpNative_Job_EntJoySample_IJobChunkMoveCompareTest_MoveJobEntityIspcMt_Execute_EntityBatchAdapter(void* context, const EntityBatchData* __batches, int __batch_start, int __batch_count)
{
    ...
    for (int __batch_index = __batch_start; __batch_index < __batch_end; ++__batch_index)
    {
        ...
        ispc::SharpNative_Job_EntJoySample_IJobChunkMoveCompareTest_MoveJobEntityIspcMt_Execute_mt_impl(position_ptr, velocity_ptr, __batchData->entityCount, DeltaTime_ptr, std::thread::hardware_concurrency());
    }
}

HEAD void* CALLINGCONVENTION Get_SharpNative_Job_EntJoySample_IJobChunkMoveCompareTest_MoveJobEntityIspcMt_Execute_EntityBatchAdapterPtr()
{
    return (void*)SharpNative_Job_EntJoySample_IJobChunkMoveCompareTest_MoveJobEntityIspcMt_Execute_EntityBatchAdapter;
}
