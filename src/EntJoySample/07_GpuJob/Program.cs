...
{
   /// <summary>
    /// 五路对比样例：同一 Job body 的 C# / C++ / ISPC / WGPU / CUDA 后端。
    ///   Move（带宽+分支）/ Heavy（16x sin/cos/sqrt 超越函数）@1M。
    ///   输出：p50 耗时 + vs C# 加速比 + 与 C# 参考的数值误差（max|diff| / 逐位不等数）。
    ///   WGPU 走 ScheduleGpu → NativeDll GpuCompute；CUDA 走 ScheduleCuda → GpuComputeCuda。
    /// </summary>
    public static class Program
    {
        ...
        {
            // ENTJOY_CUDA_ONLY=1：只跑 CUDA 验证（手写 pinned + transpiled + ScheduleCuda）
            if (Environment.GetEnvironmentVariable("ENTJOY_CUDA_ONLY") == "1")
            {
                Console.WriteLine("=== CUDA-only 验证 ===");
                try { GridSearchFullUpdate.RunCudaOnly(); }
                catch (Exception ex) { Console.WriteLine($"[FAIL] {ex}"); Environment.ExitCode = 1; }
                return;
            }

            Console.WriteLine("=== 五路对比：C# / C++ / ISPC / WGPU / CUDA（同一 Job body @1M）===");
            ...
            try
            {
                RunMoveCompare();              // C# / C++ / ISPC / WGPU / CUDA
                RunHeavyCompare();             // C# / C++ / ISPC / WGPU / CUDA
                GridSearchFullUpdate.RunGridCompare();   // GridSearch 构建/查询耗时（C#/C++/CUDA/GPU）

                // 诊断性深挖（阶段拆解/常驻/GridSearch 全流程）默认关闭，ENTJOY_GPU_DIAG=1 开启
                if (Environment.GetEnvironmentVariable("ENTJOY_GPU_DIAG") == "1")
                {
                    RunGpuStageBreakdown();
                    RunResidency();
                    GridSearchFullUpdate.Run();
                }
            }
            finally
            {
        ...
        private static void RunMoveCompare()
        {
            ...
            rows.Add(Run("CUDA (ScheduleCuda)", pos, vel, refPos, refVel,
                () => new CudaMoveJob { Positions = pos, Velocities = vel, Dt = Dt, ViewportWidth = ViewportWidth, ViewportHeight = ViewportHeight }.ScheduleCuda(N)));
        ...
        private static void RunHeavyCompare()
        {
            ...
            rows.Add(Run("CUDA (ScheduleCuda)", pos, vel, refPos, refVel,
                () => new CudaHeavyJob { Positions = pos, Velocities = vel, Dt = Dt }.ScheduleCuda(N)));
        ...
        private static unsafe void RunGpuStageBreakdown()
        {
            ...
            const uint groups = (uint)((1_000_000 + 255) / 256);
            ...
            // ④ 探针形态：vel 常驻，每帧只传 pos 8MB↑ + 8MB↓（共 16MB）
            for (int i = 0; i < Warmup; i++)
            {
                NativeTranspiler.Bindings.NativeExports.GpuCompute_WriteBuffer(b0, (IntPtr)pos.GetUnsafePtr(), size8);
                NativeTranspiler.Bindings.NativeExports.GpuCompute_Dispatch(k, bg, groups);
            ...
            Console.WriteLine();
            Console.WriteLine("  对照 CUDA 页锁定：全量 1.445ms / 探针形态(16MB) ~1.2ms / 常驻 0.244ms。");
