...
namespace EntJoySample.AutoSIMDTest
{
    public static class Program
    {
        public static void Main()
        {
            ...
            var reduceData = GenFloats(N * 100, rnd);  // SimpleReduce: A[i*100+j] needs N*100
            ...
            using var nativeReduce = new NativeArray<float>(reduceData, Allocator.Persistent);
            ...
            {
                using var r1 = NewResult(N);
                var job = new SimpleReduce_CSharp { A = nativeReduce, Result = r1 };
                return MeasurePF(ref job, N, Warmup, 10);
            }, () =>
            {
                using var r1 = NewResult(N);
                var job = new SimpleReduce_SIMD { A = nativeReduce, Result = r1 };
                return MeasureFor(ref job, N, Warmup, 10);
            }, () =>
            {
                using var r1 = NewResult(N);
                var job = new SimpleReduce_SIMD_IJob { A = nativeReduce, Result = r1, Count = N };
