...
namespace EntJoy.Debugger
{
    ...
    public static class MemoryAddress
    {
        ...
        private static readonly Dictionary<object, GCHandle> _pinned =
        ...
        public static IntPtr GetAddress(object obj)
        {
            ...
            lock (_pinned)
            {
                if (_pinned.TryGetValue(obj, out var existing) && existing.IsAllocated)
        ...
        public static IntPtr GetArrayAddress(Array array)
        {
            ...
            lock (_pinned)
            {
                if (_pinned.TryGetValue(array, out var existing) && existing.IsAllocated)
                    return existing.AddrOfPinnedObject();
                var handle = GCHandle.Alloc(array, GCHandleType.Pinned);
                _pinned[array] = handle;
                return handle.AddrOfPinnedObject();
            }
        }

        public static void ReleasePinned(object obj)
        {
            if (obj == null) return;
            lock (_pinned)
            {
                if (_pinned.Remove(obj, out var handle) && handle.IsAllocated)
                    handle.Free();
            }
        }
